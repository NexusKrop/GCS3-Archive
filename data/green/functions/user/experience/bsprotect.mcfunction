...
execute as @e[type=armor_stand,tag=building_protector] at @s run execute unless block ~ ~ ~ dark_oak_sign run scoreboard players set @s[scores={cache=1241}] cache 1245
execute as @e[type=armor_stand,tag=building_protector] at @s run execute if block ~ ~ ~ dark_oak_sign run scoreboard players set @s[scores={cache=1247}] cache 1241
execute as @e[type=armor_stand,scores={cache=1245}] at @s run tellraw @a[scores={level=3..}] [{"text": "GCS3","color": "aqua"},{"text": ">> ","color": "white","bold": true},{"text": "木牌被毁,ID","color": "white"},{"score":{"name":"@s[type=armor_stand]","objective": "protectorId"},"color": "red"},{"text": " | 最近的玩家："},{"selector":"@a[distance=..5]","color": "red"}]
