...
execute as @a at @s run execute if score @s sid = @a[scores={level=3..,viceCache=3321},limit=1] tpto run scoreboard players set @s viceCache 12356
execute unless entity @a[scores={viceCache=12356}] run tellraw @s[scores={level=3..}] [{"text": "GCS3","color": "aqua"},{"text": ">> ","color": "white","bold": true},{"text": "指定的用户不存在","color": "red"}]
